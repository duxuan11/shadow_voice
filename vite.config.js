...
function generateDataIndex() {
  ...
  for (const dirName of epDirs) {
    ...
    const topic = typeof info.topic === 'string' ? info.topic : (topics[0] || '')

    // Paths
    const videoRel = `/data/videos/${encodeURIComponent(dirName)}/video.mp4`
    const thumbRel = `/data/videos/${encodeURIComponent(dirName)}/cover.jpg`
    const videoExists = fs.existsSync(path.join(epDir, 'video.mp4'))
    const thumbExists = fs.existsSync(path.join(epDir, 'cover.jpg'))
    ...
    videos.push({
      ...
      video_url: null,
      thumbnail_local: thumbExists ? thumbRel : null,
      episode_dir: dirName,
    })
  }
